...
const TARGETS = [
  ...
  { name: "Noida", zone: "Uttar Pradesh", lat: 28.5355, lon: 77.3910 },
  { name: "Ghaziabad", zone: "Uttar Pradesh", lat: 28.6692, lon: 77.4538 },
  { name: "Faridabad", zone: "Haryana", lat: 28.4089, lon: 77.3178 },
  { name: "Greater Noida", zone: "Uttar Pradesh", lat: 28.4744, lon: 77.5080 },
  { name: "Bahadurgarh", zone: "Haryana", lat: 28.6929, lon: 76.9600 },
...
export function NavigationPanel({ curLat, curLon, speedMS, lat, lon, onSet }: { curLat: number; curLon: number; speedMS: number; lat: number; lon: number; onSet: (lat: number, lon: number) => void }) {
  ...
  const distanceKm = useMemo(() => (target ? haversineKm(curLat, curLon, target.lat, target.lon) : null), [curLat, curLon, target]);
        ...
        <CardContent className="space-y-3">
          <div className="grid sm:grid-cols-2 gap-3">
            <div>
              ...
              <Select value={selected} onValueChange={(v) => setSelected(v)}>
                <SelectTrigger className="mt-1"><SelectValue placeholder="Select a target" /></SelectTrigger>
                <SelectContent>
                  {TARGETS.map((t) => (
                    <SelectItem key={t.name} value={t.name}>{t.name} — {t.zone}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <div>
              ...
              <Input className="mt-1" type="number" value={(target?.lat ?? lat)} onChange={(e)=>onSet(Number(e.target.value), target?.lon ?? lon)} />
            </div>
            <div>
              <Label>Longitude</Label>
              <Input className="mt-1" type="number" value={(target?.lon ?? lon)} onChange={(e)=>onSet(target?.lat ?? lat, Number(e.target.value))} />
            </div>
          </div>

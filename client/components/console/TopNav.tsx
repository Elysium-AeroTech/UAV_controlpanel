...
export function TopNav({ status, commanderName }: { status: string; commanderName?: string }) {
  return (
    <header className="sticky top-0 z-30 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-b">
      <div className="container mx-auto px-4 h-14 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 text-xs text-muted-foreground">
          {commanderName ? <div className="hidden md:flex items-center gap-1"><GaugeCircle className="h-4 w-4" /> {commanderName}</div> : null}

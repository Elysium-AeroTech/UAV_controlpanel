...
export function ManualControlPanel({
  ...
  const doEmergencyAbort = () => {
    if (
      !confirm(
        "Confirm EMERGENCY ABORT? This will kill motors and deploy parachute.",
      )
    )
      return;
    setAborted(true);
        ...
        {!unlocked ? (
          <div className="space-y-3">
            <div className="text-sm text-muted-foreground">
              Enter Commander's Code to unlock manual controls.
            </div>
            <div className="flex gap-2 max-w-md">
              <Input
                placeholder="Commander's Code"
                value={code}
              ...
              {aborted && (
                <div className="rounded border border-red-500/50 bg-red-500/10 p-3 text-sm">
                  ...
                  <div className="mt-1 text-red-300">
                    Motors: {motorsKilled ? "KILLED" : "ACTIVE"} • Parachute:{" "}
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="p-3 rounded border bg-background">
                  ...
                  <div className="font-mono text-lg">
                    {aborted ? 0 : kmph.toFixed(0)} km/h
                  </div>
                  <div className="font-mono text-xs">
                    Mach {aborted ? 0 : mach.toFixed(2)}
                  </div>
                </div>
                <div className="p-3 rounded border bg-background">
                ...
                <div className="p-3 rounded border bg-background">
                  ...
                  <div
                    className={`font-mono text-lg ${aborted ? "text-red-400" : "text-emerald-400"}`}
                  >
                    {aborted ? "ABORTED" : "Nominal"}
                  </div>
                </div>
              </div>

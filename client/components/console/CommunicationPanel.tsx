...
export function CommunicationPanel() {
  const [status, setStatus] = useState<"Connected" | "Disconnected" | "Attempting">("Connected");
      ...
      <Card>
        ...
        <CardContent className="grid sm:grid-cols-2 gap-3 text-sm">
          <div className="p-3 rounded border bg-background">
            ...
            <Badge variant={status === "Connected" ? "secondary" : status === "Attempting" ? "outline" : "destructive"}>{status}</Badge>
          </div>
          <div className="p-3 rounded border bg-background">

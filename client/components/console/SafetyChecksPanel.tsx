...
export function SafetyChecksPanel({ verified, setVerified }: { verified: boolean; setVerified: (b: boolean) => void }) {
  const [progress, setProgress] = useState<Record<CheckKey, number>>(() => Object.fromEntries(CHECKS.map((c) => [c, 0])) as any);
  ...
  return (
    <div className="grid md:grid-cols-2 gap-4">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Wrench className="h-5 w-5 text-primary" /> Safety Checks</CardTitle>
        </CardHeader>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          {verified ? <div className="text-emerald-400 flex items-center gap-2"><CheckCircle2 className="h-4 w-4" /> Safety Verified</div> : null}
          <Separator />
          <div className="text-xs text-muted-foreground">Proceed to Arming once verified.</div>
        </CardContent>
      </Card>
    </div>
  );
